...
int * apply_all(int *array1, const size_t array1_size, int *array2, const size_t array2_size)
{
    ...
    int *products = new int[total_size];
    
    if (!products) // Check if Heap Allocation was successful
        return nullptr;
    ...
    for(size_t i {0}; i < array2_size; i++)
        for(size_t j {0}; j < array1_size; j++)
            products[index++] = array2[i] * array1[j];
    
    return products;
}

void print(int *results, const size_t results_size)
{
    cout << endl << "[ ";
